...
class User implements UserInterface
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=Librairies::class, mappedBy="createdBy")
     */
    private $librairies;
...
//        return $this->getUserIdentifier();
    ...
    public function addLibrairy(Librairies $librairy): self
    {
        if (!$this->librairies->contains($librairy)) {
            $this->librairies[] = $librairy;
            $librairy->setCreatedBy($this);
        }

        return $this;
    }

    public function removeLibrairy(Librairies $librairy): self
    {
        if ($this->librairies->removeElement($librairy)) {
            // set the owning side to null (unless already changed)
            if ($librairy->getCreatedBy() === $this) {
                $librairy->setCreatedBy(null);
            }
        }
